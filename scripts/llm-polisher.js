...
async function polishText(rawText, targetPillar) {
  console.log(
    '[LLM-POLISHER] Passing raw text to inference cascade for tactical metadata on pillar:',
    targetPillar,
  );
  ...
  try {
    ...
    if (targetPillar === 'esim_data_plans') {
      pillarContext =
        'FOCUS: Emphasize global connectivity, B2B enterprise roaming, and latency reduction solutions.';
    } else if (targetPillar === 'reputation_management') {
      pillarContext =
        'FOCUS: Emphasize Google Local Search Authority, review gating, and brand trust automation.';
...
`;
    ...
    console.error(
      '[LLM-POLISHER] API cascade failed, falling back to basic extraction:',
      err.message,
    );
    ...
    const fallbackContent = contentMatch ? contentMatch[1].trim() : 'No content available.';
  ...
  try {
    ...
    console.warn(
      '[LLM-POLISHER] AGC Rewriting pipeline failed. Skipping paraphrase merging.',
      rewriteErr.message,
    );
  }
